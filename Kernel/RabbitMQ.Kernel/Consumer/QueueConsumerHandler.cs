...
namespace RabbitMQ.Kernel.Consumer
{
    ...
    public class QueueConsumerHandler<TMessageConsumer, TQueueMessage> : IQueueConsumerHandler<TMessageConsumer, TQueueMessage> where TMessageConsumer : IQueueConsumer<TQueueMessage> where TQueueMessage : class, IQueueMessage
    {
        ...
        private readonly IChannelProviderStrategyFactory _ChannelProviderStrategyFactory;
        ...
        public QueueConsumerHandler(IServiceProvider serviceProvider,
            ...
            IQueueNameResolver queueNameResolve,
            IChannelProviderStrategyFactory channelProviderStrategyFactory,
            QueueDefinition queueDefinition)

        {
            _serviceProvider = serviceProvider;
            _logger = logger;
            _ChannelProviderStrategyFactory = channelProviderStrategyFactory;
            _consumerName = typeof(TMessageConsumer).Name;
            _queueDefinition = queueDefinition;
            _queueName = queueDefinition.QueueName ?? queueNameResolve.Resolve<TQueueMessage>();
        }
        ...
        public void RegisterQueueConsumer()
        {
            ...
            _consumerRegistrationChannel = _ChannelProviderStrategyFactory.Create<TQueueMessage>(_queueDefinition.ExchangeType).GetChannel(_queueName);
            //  _consumerRegistrationChannel = scope.ServiceProvider.GetRequiredService<IQueueChannelProvider<TQueueMessage>>().GetChannel(_queueName);
